...
## This is Quintom String cosmology
# with, maybe, the interaction term
...
class QuintomStringCosmology(LCDMCosmology):
    def __init__(self, vary_k1=False, vary_k2=False, vary_iniphi=False, vary_coupling=False, vary_Ok=False):

        """
        It better to start the chains at masses equal one, othewise may take much longer.
        k1 - parameter describing the potential, for quadratic terms -> mass of Quintessence.
        k2 - parameter describing the potential, for quadratic terms -> mass of Phantom.
        """

        self.vary_k1 = vary_k1
        self.vary_k2 = vary_k2
        self.vary_coupling = vary_coupling
        self.vary_iniphi = vary_iniphi
        self.vary_Ok = vary_Ok


        self.k1 = 0 if (vary_k2 and (not vary_k1)) else k1_par.value
        self.k2 = 0 if (vary_k1 and (not vary_k2)) else k2_par.value
    ...
    def freeParameters(self):
        ...
        if self.vary_k1: l.append(k1_par)
        if self.vary_k2: l.append(k2_par)
    ...
    def updateParams(self, pars):
        ...
        if self.chatty:
            print('-'*10)
            print ('mphi={}, mphan={}, coupling={}'.format(self.k1, self.k2, self.coupling))
    ...
    def sf_potential(self, phi, psi, select):
        ...
        if select == 0:
            potential = 0.5*(self.k1*phi)**2 + 0.5*(self.k2*psi)**2 + self.coupling*(phi*psi)**2
        elif select == 'phi':
            potential = self.k1**2*phi + 2*self.coupling*phi*psi**2
        elif select == 'psi':
            potential = self.k2**2*psi + 2*self.coupling*psi*phi**2
        return potential
    ...
    def compute_omega_de(self, ini_guess):
        ...
        if self.vary_k1 and (self.k2 == 0) and (self.coupling == 0):
            phi_ini, psi_ini = 10**ini_guess, 0
        elif self.vary_k2 and (self.k1 == 0) and (self.coupling == 0):
    ...
    def find_initial_phi(self, low_guess=-2, high_guess=3, tolerance=5E-3):
        ...
        if self.chatty:
            if mid_guess == -1:
                print('looks a lot like LCDM', mid_guess, self.k1, self.k2, self.iniphi)
        if mid_guess == 0:
            if self.chatty: print('-- No solution found!', mid_guess, self.k1, self.k2, self.iniphi)
        return mid_guess, solution
