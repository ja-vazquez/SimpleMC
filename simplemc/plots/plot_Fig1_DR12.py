...
plot_errorbar(zLyaA,  11.28*(1+zLyaA),  yerr=0.65*(1 + zLyaA),  color='red', fmt='o', markersize=8,
              label="$\\rm{BOSS}\ \\mathrm{Ly}\\alpha\\mbox{-}\\rm{auto}\ \\rm{DR11}$", empty=empty2)
plot_errorbar(zLyaA,  9.18*zLyaA,       yerr=0.28*zLyaA,
              color='green', fmt='-o', markersize=8, empty=empty2)

plot_errorbar(zLyaC,  10.8*(1+zLyaC),   yerr=0.4*(1+zLyaC),    color='red', fmt='*', markersize=8,
              label="$\\rm{BOSS}\ \\mathrm{Ly}\\alpha\\mbox{-}\\rm{cross}\ \\rm{DR9}$", empty=empty2)
