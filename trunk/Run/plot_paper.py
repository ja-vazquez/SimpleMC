...
###################    Extra
  ...
  if l=='smoking' or l=='decay':
	...
	params   =  ['lambdaf']
	leg=['$f_x=1$','$f_x=0.5$','$f_x=0.1$']
	T.Plotting_1d(dire, model, extra, datasetl, params,Nb=[60,60,58],legend=leg)
        pylab.tight_layout()
  ...
  if l=='decay2D':
	extra    = 'phy'
	datasetl = ['BBAO+Planck']
	model    =  ['Decay_z2','Decay05_z2','Decay01_z2']
	leg=['$f_x=1$','$f_x=0.5$','$f_x=0.1$']

	para=['Om2'] #,'h','Omh2']

	for paral in para:
  	   params   =  ['lambdaf',paral]
	   T.Plotting_2d(dire, model, extra, datasetl, params, legend=leg, Nb=20)
	   pylab.tight_layout()
	   pylab.savefig('decay_2D_'+paral+'.pdf')
	   if show: pylab.show()
