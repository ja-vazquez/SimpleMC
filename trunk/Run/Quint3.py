...
def solver(x0):
    phidot0=sqrt(2.0*((1-Om0)*exp(-3*lna_ini)-Pot(0,0)))
    print phidot0
    y0= [0, phidot0]
...
lna = np.linspace(lna_ini, 9, 1000)
...
ax2.plot(lna, (0.5*yy**2+Pot(xx,0))*(H_0/hub(exp(lna), sol.T))**2)
ylabel("$\\Omega_{\phi}$")
xlabel("$\ln a$")
