...
class plot_aux():
  ...
  def Plotting_2d(self, dire, modell, extra, datasetl, param, Nb=50, Ol=False, lwidth=None,loc=None,fillcolor=None, legcolor=False, legend=None): 
	...
	ax = fig.add_subplot(111, aspect='equal')
        ...
        for model in modell:
		 ...
		 pass
	       try:
	         C['Omh2']=C['Om']*C['h']**2
	         C.lname['Omh2']='$\Omega_m h^2$'
	       except:
		 pass
	       try:
		 C['Om_2']=C['Om']*(3.0)**3	
		 C.lname['Om_2']='$\Omega_m (z=2.0)$'
	       except:
		 pass 	
		...
		fcol=self.colour(a)		     	
	     ...
	     C.Plot2D(param[0], param[1],filled=fcol, lw=lwh, label=label, N=Nb)

	     plot_ellipse = True
	     if plot_ellipse:
	        mn,cov= C.GetCovariance([3,4])	
 	        val,vec = linalg.eig(cov)
	        vec=vec.T
		
                with open('book_cont.txt', 'a') as f:
                        f.write('%s_%s_%s\n'%(model,extra,dataset))
			f.write('[Om, Ol ] \n')
			f.write(' mean, vect1, vect2\n')
                        for ss in [2.3, 5.99, 11.83]:
                                f.write('%s\t %s\t %s\n'%(mn,vec[0]*sqrt(ss*real(val[0])),vec[1]*sqrt(ss*real(val[1])))) 
			f.write('\n')
	
	        vec[0]*=sqrt(11.83*real(val[0]))
	        vec[1]*=sqrt(11.83*real(val[1]))


	        pylab.plot(mn[0],mn[1],'bo')
	        pylab.plot([mn[0]-vec[0][0],mn[0]+vec[0][0]],
                  [mn[1]-vec[0][1],mn[1]+vec[0][1]],'r-')
	        pylab.plot([mn[0]-vec[1][0],mn[0]+vec[1][0]],
                  [mn[1]-vec[1][1],mn[1]+vec[1][1]],'r-')
		   ...
		   return vals[order], vecs[:,order]
	        ...
	        for sigs in sigmas:
	           ...
	           ell = Ellipse(xy=(mn[0], mn[1]),  width = w, height = h, angle=theta, color='green',  lw=4) 	
