...
def Pot(x, i):
    if i==0: 
	return V0*exp(-lam*x)*Mpl**4
    if i==1: 
	return -lam*V0*exp(-lam*x)*Mpl**4
    else:
	print 'wrong choice' 
	stop()
...
def hub(a, x_vec):
    x, y  = x_vec
    return H_0*sqrt(0.5*y**2 + Pot(x,0)/(3*H_0**2*Mpl**2) + Om0/a**3  + Or0/a**4)	


def RHS(x_vec, lna):
    a=exp(lna)	
    x, y = x_vec
    return [3.0*y*H_0/hub(a,x_vec), -3*y -Pot(x,1)/(3*Mpl**2*H_0*hub(a,x_vec))]
...
ax.plot(lna, (0.5*yy**2+Pot(xx,0)/(3*H_0**2*Mpl**2))*(H_0/hub(exp(lna), sol.T))**2, 'k-')
...
ax2.plot(lna, (0.5*yy**2*9*H_0**2-Pot(xx,0))/(0.5*yy**2*9*H_0**2+Pot(xx,0)), 'k-')
ylabel("$\phi/H_0$")
xlabel("$\ln a$")
pylab.xlim([-13,0])
