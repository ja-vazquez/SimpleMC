...
class Sampler(object):
    ...
    def __init__(self, loglikelihood, prior_transform, npdim, live_points,
                 ...
                 queue_size, pool, use_pool): #jav, outputname = "chains/dynestySamples"):
        ...
        #print("outputname {}, type : {}".format(self.outputname, type(outputname)))
    ...
    def run_nested(self, maxiter=None, maxcall=None, dlogz=None,
                   ...
                   print_func=None, save_bounds=True, outputname= None):
        """
        **A wrapper that executes the main nested sampling loop.**
        ...
        try:
            ...
            for it, results in enumerate(self.sample(maxiter=maxiter,
                ...
                if print_progress:
                    i = self.it - 1
                    # logwt -> results[5], logstar -> results[3]
            ...
            if add_live:
                ...
                for i, results in enumerate(self.add_live_points()):
                    ...
                    #f = open(self.outputname + '_tmp.txt', 'w+')
                    #weights = np.exp(results[5])
                    #vstarstr = str(results[2]).lstrip('[').rstrip(']')
                    #f.write("{} {} {}\n".format(weights, -1*results[3], vstarstr))
                    #f.flush()
